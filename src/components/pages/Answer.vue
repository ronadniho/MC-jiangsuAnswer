        ...
        <div class="result">
          ...
          <div class="result-item" v-for="(item,index) in examlist" :key="item.ID">
            <div v-if="index+1==page">
              ...
              <!--<div class="tip" >&lt;!&ndash;单选题或判断题&ndash;&gt;-->
              <!--<span>-->
              <!--提示&nbsp;&nbsp;&nbsp;&nbsp;-->
              <!--<b>{{item.PROMPT}}</b>-->
              <!--</span>-->
...
<script>
   ...
   */
    created: function () {
      ...
      /* var localQue = this.getSession(this.user_id + '_everyday');

       var callback = () => {
         if (localQue) {//有缓存
           console.log('有缓存')
           console.log(localQue)
           console.log(localQue[0].CREATE_TIME)
           console.log(this.CREATE_TIME)
           if (localQue[0].CREATE_TIME == this.CREATE_TIME) {//说明本地存的试题是当天的不需要清除
             alert('当天的数据')
           }
           else {//说明本地存的试题不是当天的需要清除
             alert('清除本地旧数据')
             this.removeSession(this.user_id + '_everyday');
           }
           /!*this.examlist = localQue;
           this.totalPage = localQue.length;
           this.btnDisabled = !localQue[0].DISABLED;
           this.loading = false;*!/
         }
         else {//无缓存
           console.log('无缓存')
       ...
       this.getClassExamlist(this.user_id, callback);*/
      this.getClassExamlist(this.user_id);
